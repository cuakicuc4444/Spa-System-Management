...
export default function BookingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const hasServices = guestServices.some(
    ...
    const roundedInvoiceItems = guestServices
      .flatMap((serviceIds, index) =>
    ...
    const fullPhoneNumber = `${dialCode} ${formData.phone.trim()}`;
  ...
  const totalUSD = allSelected.reduce(
    (sum, t) => sum + Number(t.priceUSD || 0),
    0
  );
